...
; edited by X. Feng xfeng17@jhu.edu to select spots detected in the 2nd and 3rd channels in the first 20 frames (changes were commented 'XF')
; for example, if imaging Cy3, Cy5 and Cy7, spots detected in both Cy5 AND Cy7 emission channels are selected (regardless of their Cy3 status)
...
;	endfor
	...
	for j = 10, 19 do begin
		readu, 1, frame
		frame_average_second = temporary(frame_average_second) + frame
	endfor

	for j = 20, 29 do begin
		readu, 1, frame
		frame_average_first = temporary(frame_average_first) + frame
	endfor

    frame_average_first = temporary(frame_average_first)/float(10)
	frame_average_second = temporary(frame_average_second)/float(10)
	frame_average_third = temporary(frame_average_third)/float(10)
...
;	endfor
    ...
    temp_first = temporary(temp_first)/float(10)
	temp_second = temporary(temp_second)/float(10)
    temp_third = temporary(temp_third)/float(10)
...
;  WIDGET_CONTROL, text_ID, SET_VALUE=("deviation, cutoff :" + STRING(deviation)+ STRING(float(cutoff))), /APPEND, /SHOW
	...
	for d = 0, 2 do begin
		c = 2 - d ; analyze channel 3, 2 then 1 - XF 01/13/2020
				...
				if truncated_frame(i,j) gt 0 then begin
					...
					if (Max_location_x eq 0) and (Max_location_y eq 0) then begin
	                    ...
	                    if (quality lt quality_num) and (occupy gt occupy_num) then begin
                            ...
                            ; filter for spots that colocalize in second and thrid channels - XF 01/13/2020
							all_check = 'n'
							if c eq 1 then begin
								;for g = 0, NumberofGoodLocations_second - 1 do begin
								  ;if (abs(GoodLocations_x_second(g)-Max_location_x) le 2)and(abs(GoodLocations_y_second(g)-Max_location_y) le 2) then begin
								    for h = 0, NumberofGoodLocations_third - 1 do begin
								      if (abs(GoodLocations_x_third(h)-Max_location_x) le 2) and (abs(GoodLocations_y_third(h)-Max_location_y) le 2) then begin
                                        all_check = 'y'
                                      endif
                                    endfor
                                  ;endif
